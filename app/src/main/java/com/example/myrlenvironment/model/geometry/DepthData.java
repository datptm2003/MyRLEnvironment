...
public class DepthData {
    ...
    public static FloatBuffer create(Frame frame, Anchor cameraPoseAnchor) {
        try {
            ...
            Log.d("DepthData", depthImage.getWidth() + " * " + depthImage.getHeight());
    ...
    private static FloatBuffer convertRawDepthImagesTo3dPointBuffer(
        ...
        final float maxNumberOfPointsToRender = 2048;
        int step = (int) Math.ceil(Math.sqrt(depthWidth * depthHeight / maxNumberOfPointsToRender));

        int pointWidth = (int) Math.ceil(depthWidth * 1.0 / step);
        int pointHeight = (int) Math.ceil(depthHeight * 1.0 / step);
        FloatBuffer points = FloatBuffer.allocate(pointWidth * pointHeight * FLOATS_PER_POINT);
